...
export const EventPremiumChip = ({ event }: { event: Partial<RestItemEvent> }) => {
  ...
  const isInvitation = isEventInvitation(event)

  return [isAdh, isAdhDues, isPrivate, isInvitation].some(Boolean) ? (
    <XStack testID="event-premium-chip">
      <VoxCard.AdhLock lock={isLocked} due={isAdhDues} isPrivate={isPrivate} isInvitation={isInvitation} />
    </XStack>
  ) : (
    false
  )
