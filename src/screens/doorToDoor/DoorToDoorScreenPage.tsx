...
type DoorToDoorScreenPageProps = {
  embeddedInLayout?: boolean
}

const DoorToDoorScreenPage = ({ embeddedInLayout = false }: DoorToDoorScreenPageProps) => {
  ...
  const navigateToBuildingDetail = useCallback(
    (id: string) => {
      ...
      if (address?.building.campaignStatistics) {
        setAddress(address)
        router.push('/old/porte-a-porte/building-detail')
      }
    },
  ...
  const filterTopPadding = embeddedInLayout ? 0 : insets.top

  const renderMap = useCallback(
    (initalLocation?: LatLng) => (
      <>
        <YStack style={styles.filter} pt={filterTopPadding}>
    ...
    [displayMode, filter, filterTopPadding, filteredAddresses, handleCampaignRankingSelected, isLoading, navigateToBuildingDetail, setCurrentSearchRegion],
  )
...
export default DoorToDoorScreenPage
