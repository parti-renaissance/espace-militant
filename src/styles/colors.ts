export const white = '#ffffff'
export const black = '#000000'
const springWood = '#f9f6f4'

export const shipGray = '#413d45'
const ebony = '#10152F'
const boulder = '#767676'
const cararra = '#eae8e4'
const cinnarbar = '#e74536'
const lightGrey = '#e7e9ec'
const doveGray = '#6a6a6a'
const anakiwa = '#b1d8ff'
const shakespeare = '#00AEEF'
const anakiwa2 = '#B1D8FF'
const monza = '#CC0613'
const portGore = '#1D214F'
const comet = '#616481'
const pickledBluewood = '#383b64'
const selago = '#F2F5FE'

export const darkText = ebony
export const lightText = boulder
export const veryLightText = white
export const lightTextOnLightBackground = doveGray
export const titleText = portGore
export const tagBackground = anakiwa2
export const defaultBackground = white
export const touchHighlight = '#dddddd'
export const navigationBackground = white
export const navigationTint = portGore
export const secondaryButtonText = shipGray
export const secondaryButtonBackground = selago
export const tertiaryButtonBackground = white
export const progressBackground = cararra
export const inputTextBorder = '#eaeaea'
export const loadingOverlayBackground = '#00000033'
export const inputTextErrorMessage = cinnarbar
export const modalOverlayBackground = '#00000066'
export const groupedListBackground = springWood
export const separator = lightGrey
export const activeItemBackground = selago
export const primaryColor = shakespeare
export const accent = shakespeare
export const primaryDisabledBackground = '#B2E7FA'
export const primaryHighlightBackground = '#80D6F7'
export const secondaryDisabledBackground = '#FBFCFE'
export const secondaryHighlightBackground = '#F8FAFE'
export const tabBarBackground = black
export const tabBarActiveTint = white
export const tabBarInactiveTint = comet
export const tabBarRipple = pickledBluewood
export const lightBackground = selago
export const primaryButtonTextColor = white
export const coloredText = shakespeare
export const quickPollProgress = anakiwa
export const actionCardBackground = portGore
export const actionCardHighlight = pickledBluewood
export const accountCardBackground = selago
export const toolsCardBackground = selago
export const circularIconBackground = selago
export const highlightedNewsBackground = selago
export const frenchFlagBlue = portGore
export const frenchFlagWhite = white
export const frenchFlagRed = monza
export const homeBannerOverlayBackground = 'rgba(0, 0, 0, 0.4)'
export const homeFeedEventBackground = portGore
export const homeFeedActionBackground = monza
