...
import type { RestFullEvent, RestItemEvent, RestPartialEvent, RestPublicItemEvent } from './schema'
...
export const groupEventsBySection = <T extends RestItemEvent | RestPublicItemEvent>(
  events: T[],
  options?: GroupEventsBySectionOptions,
): EventSection<T>[] => {
...
export const isEventAdherentReserved = (event: Partial<RestItemEvent>) => event.visibility === 'adherent'

export const isEventAdherentDuesReserved = (event: Partial<RestItemEvent>) => event.visibility === 'adherent_dues'

export const isEventPrivate = (event: Partial<RestItemEvent>) => event.visibility === 'private'

export const isEventInvitation = (event: Partial<RestItemEvent>) => event.visibility === 'invitation'
...
export const isEventDetailsLoading = (event: Partial<RestItemEvent>, isFetching = false) =>
  Boolean(isEventPartial(event) && isFetching)

export const isEventEditorKnown = (event: Partial<RestItemEvent> & { editable?: boolean }) =>
  Boolean(event.editable && !isEventCancelled(event))
...
export const isEventToggleRegisterHided = (event: Partial<RestItemEvent>, userUuid?: string) =>
  [isEventUserAuthor(event, userUuid)].some(Boolean)
...
export const shouldUseLockImageFallback = (event: Partial<RestItemEvent>) =>
  isEventPartial(event) &&
  [isEventAdherentReserved(event), isEventAdherentDuesReserved(event), isEventPrivate(event), isEventInvitation(event)].some(Boolean)
