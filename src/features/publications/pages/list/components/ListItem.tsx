...
import { SenderView } from '@/features/publications/components/SenderView'
      ...
      <VoxCard.Content gap={20}>
            ...
            {item.status === 'sent' && item.sent_at ? (
              <Text.MD secondary>
                ...
                <Text.MD secondary semibold>
                  {relativeDateFormatter(item.sent_at)}
                </Text.MD>{' '}
                par{' '}
                <Text.MD secondary semibold>
              ...
              <Text.MD secondary>
                Créée le{' '}
                <Text.MD secondary semibold>
                  {relativeDateFormatter(item.created_at)}
                </Text.MD>{' '}
                par{' '}
                <Text.MD secondary semibold>
                  {item.author.first_name} {item.author.last_name}
        ...
        {item.status === 'draft' ? (
          <XStack gap="$small" justifyContent={media.md ? 'space-between' : 'flex-start'}>
            <XStack gap="$small">
              <VoxButton variant="outlined" theme="gray" iconLeft={Pen} size="md" onPress={handleEdit}>
                Modifier
              </VoxButton>
            </XStack>
            <XStack gap="$small">
          ...
          <XStack gap="$small" justifyContent={media.md ? 'space-between' : 'flex-start'}>
            <XStack gap="$small">
              <VoxButton variant="outlined" theme="gray" iconLeft={Eye} size="md" onPress={handleView}>
                Voir
              </VoxButton>
              <VoxButton variant="outlined" theme="gray" iconLeft={ChartColumn} size="md" onPress={handleStats}>
                Stats
              </VoxButton>
            </XStack>
            <XStack gap="$small">
