...
export default function MessagePageIndex() {
  ...
  const availableScopes = useMemo(() => (scopes?.list || []).filter((scope) => scope.features.includes('publications')), [scopes?.list])
  ...
  const scopeOptions = useMemo(() => availableScopes.map((scope) => ({ value: scope.code, label: scope.name })), [availableScopes])
  ...
  return <IndexContent scopeOptions={scopeOptions} selectedScope={selectedScope} onScopeChange={handleScopeChange} />
}
