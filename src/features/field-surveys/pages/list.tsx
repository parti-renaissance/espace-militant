...
import { useProfileCompletionAccess } from '@/features/profil/hooks/useProfileCompletionAccess'
...
const SurveyListItem: React.FC<SurveyListItemProps> = ({ survey, onPress, showBorder = true }) => {
  ...
  return (
    <YStack padding="$medium" borderBottomWidth={showBorder ? 1 : 0} borderBottomColor="$textOutline" flex={1} gap="$medium">
      {getSurveyTypeChip()}
      ...
      {survey.city && (
        <XStack alignItems="center" gap="$xsmall" marginBottom="$xsmall">
          <Text.SM secondary>{survey.city}</Text.SM>
        </XStack>
      )}
      ...
      <XStack alignItems="center" gap="$medium">
        ...
        {(survey as any)?.created_at && (
          <XStack alignItems="center" gap="$xsmall">
            ...
            <Text.SM secondary>Crée le : {getFormattedDate(new Date((survey as any).created_at))}</Text.SM>
          </XStack>
        )}
      </XStack>
        ...
        <YStack>
          {media.gtSm
              ...
              surveys.map((survey, index) => {
                ...
                if (index % 2 === 0) {
                  return (
                    <GridRow key={index}>
                      <GridItem>
                        <SurveyListItem survey={survey} onPress={handleSurveyPress} showBorder={showBorder} />
                      </GridItem>
                      <ColumnSeparator />
                      <GridItem>
                        {surveys[index + 1] ? (
                          <SurveyListItem survey={surveys[index + 1]} onPress={handleSurveyPress} showBorder={showBorder} />
                        ) : (
                          <YStack flex={1} />
                        )}
                      </GridItem>
            ...
            : surveys.map((survey, index) => {
                ...
                return <SurveyListItem key={survey.uuid} survey={survey} onPress={handleSurveyPress} showBorder={showBorder} />
              })}
        </YStack>
      </VoxCard.Content>
    </VoxCard>
    ...
    >
      <VoxCard.Content>
        <YStack alignItems="center" justifyContent="center" padding="$xxlarge" gap="$xlarge">
          ...
          <Text.LG semibold primary textWrap="balance" maxWidth={480} textAlign="center">
            Il n'y a actuellement aucun questionnaire en cours sur votre zone.
          </Text.LG>
