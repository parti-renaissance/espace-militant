...
import { RestGetMessageContentResponse, RestGetMessageResponse } from '@/services/messages/schema'
...
interface MessageDetailsScreenSkeletonProps { }

export const HeaderMesssageDetails = () => {
...
export const MessageDetailsScreenSkeleton: React.FC<MessageDetailsScreenSkeletonProps> = () => {
  return (
...
const MessageDetailsScreen: React.FC<MessageDetailsScreenProps> = ({ data, content, isLoading, error }) => {
  if (isLoading) {
    ...
    <ScrollView backgroundColor="$surface" flex={1} contentContainerStyle={{ paddingBottom: 100 }}>
      <YStack gap="$medium" $gtSm={{ maxWidth: 600, width: '100%', marginHorizontal: 'auto'}}>
        ...
        <PublicationCard
          ...
          title={data.subject}
          description={content?.json_content}
          author={data.sender}
          uuid={data.uuid}
        />
      </YStack>
