...
import MessageDetailsScreen, { MessageDetailsScreenDeny, MessageDetailsScreenSkeleton, HeaderMesssageDetails } from '@/features/message/pages/detail/MessageDetailsScreen'
import { useGetMessage, useGetMessageContent } from '@/services/messages/hook'
...
function MessageDetailScreen(props: Readonly<{ id: string }>) {
  ...
  const { data: messageContent, isLoading: isMessageContentLoading, error: messageContentError } = useGetMessageContent({
    messageId: props.id,
    scope: defaultScope!,
    enabled: Boolean(defaultScope)
  })
  ...
  return (
    <>
      ...
      <MessageDetailsScreen data={messageData} content={messageContent} isLoading={isMessageLoading || isMessageContentLoading} error={messageError || messageContentError} />
    </>
  )
}
