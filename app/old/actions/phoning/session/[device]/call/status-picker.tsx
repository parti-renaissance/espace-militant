...
const PhoneCallStatusPickerScreen = () => {
  ...
  const sendStatusCodeAndLeave = (statusCode: string) => {
    ...
    PhoningCampaignRepository.getInstance()
      ...
      .then(() => {
        router.replace({
          pathname: '/old/actions/phoning/session/[device]/call/failure',
          params: { device },
        })
      })
      .catch((error) => {
  ...
  const onAction = (statusCode: string) => {
    if (statusCode === 'answered') {
      router.replace({
        pathname: '/old/actions/phoning/session/[device]/poll/detail',
        params: { device },
      })
    } else {
  ...
  const Content = (callStatuses: Array<PhoningSessionCallStatus>) => {
    ...
    return (
      <View style={styles.content}>
        <Stack.Screen options={{ title: campaign?.title || 'N/A' }} />
        <FlatList
          ...
          ListHeaderComponent={() => {
            return (
              <>
                <Text style={styles.title}>{session?.adherent?.info || 'Aucune information disponible'}</Text>
